...
describe("UpdateRide", () => {
  ...
  it("should return a ride updated when updateRideRepository insert it", async () => {
    const ride = await testInstance(fakeQuery, fakeRideEntity);
    expect(ride).toEqual(fakeRideEntity);
  });
  it("should return null a new ride updated when updateRideRepository return it", async () => {
